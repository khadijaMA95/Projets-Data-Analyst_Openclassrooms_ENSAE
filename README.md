...
:black_medium_small_square:	 Réaliser l'algorithme des k-means.

:black_medium_small_square:	 Optimiser un modèle avec GridSearchCV.

:black_medium_small_square:	 Valider un modèle avec la validation croisée.
...
:black_medium_small_square:	 Préparer et analyser les données comprenant les caractéristiques dimensionnelles des billets en fonction de leur authenticité.

:black_medium_small_square:	 Explorer différentes solutions pour la mise en place de l'algorithme et retenir le modèle le plus adapté.

:black_medium_small_square:	 Tester notre algorithme sur de nouvelles données.

:file_cabinet: Projet 9 : Produisez une étude de marché avec Python

🎓 Compétences acquises :

:black_medium_small_square:	 Effectuer un clustering simple.
...
:black_medium_small_square:	Préparer, nettoyer et réaliser une analyse exploratoire dans un notebook Juypter :
...
:black_medium_small_square:	 Réaliser le clustering des pays grâce à la classification ascendante hiérarchique (CAH) et à la méthode des k-means puis réaliser une analyse en composantes principales (ACP) dans un second notebook.
...
:black_medium_small_square:	 Obtenir un jeu de données exploitables à partir des fichiers source, puis réaliser une analyse complète : chiffres clés, KPI, graphiques, etc.. Nous réalisons également plusieurs tests statistiques afin de vérifier la corrélation entre différentes variables liées au profil des clients.
...
:black_medium_small_square:	 Obtenir un jeu de données cohérent et complet à partir des fichiers qui nous ont été fournis puis étudier les ventes de la boutique en ligne. Il faudra également réaliser une étude univariée sur les prix de ventes des produits.
...
:black_medium_small_square:	 Créer le dictionnaire des données et le schéma relationnel normalisé.

:black_medium_small_square:	 Modifier la base de données permettant de collecter les transactions immobilières et foncières en France, puis implémenter cette nouvelle base de données et enfin effectuer les requêtes SQL nécessaires aux analyses.
